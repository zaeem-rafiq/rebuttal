...
The September 7 report remains unchanged as a historical artifact from the earlier rubric. Its results must not be attributed to grounded-v2. **Current grounded-v2 model evaluation: completed September 14, 2026.** Following the systematic reconciliation of intake/eval fixtures, elimination of hardcoded case prompts, and enforcement of strict factual grounding rules across the multi-agent graph, the generalized pipeline achieved 20/20 action matches (target $\ge 18$), 20/20 gate matches (target $20/20$), 20/20 EV-sign matches (target $20/20$), and 17–19/20 narrative judge passes (target $\ge 18$). See [generalized results](../evals/results/2026-09-14-generalized.md), [failure adjudication analysis](evals-adjudication.md), and historical [initial baseline](../evals/results/2026-09-14.md).

Factual grounding rules strictly enforce zero-editorial-gloss, prospective recommendation framing, attributed message quotes, and mathematical currency/timestamp equivalence without any golden set case references. Failure modes identified during evaluation runs were systematically categorized and addressed via principled domain prompt engineering rather than prompt-cheating heuristics. No thresholds or case labels were weakened.
