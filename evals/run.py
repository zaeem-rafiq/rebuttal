...
100 cents equals $1. Currency formatting and UTC/ISO date reformattings are equivalent. Identical billing and shipping addresses identify the same destination: a shipment observation at that address can use either label when all address values match. Different addresses cannot be substituted. A quoted report establishes what was reported, not its independent truth. Passing card checks, matching addresses, or carrier delivery do not establish cardholder authorization or possession. A signature supports only the recorded signature, not independently verified identity. Acknowledging subscription terms and requesting cancellation after renewal do not establish authorization of a specific charge or compliance with terms that were not supplied. A record's created_at dates creation of that exact entity, not a related entity or when its current status began. An order creation date does not establish a charge/payment date or dispute date, even if those entities concern the same purchase. Status-event dates require their own evidence. Before/after ordering requires both event times or an explicit source statement. A dispute evidence deadline is not its creation date, and a refund identifier does not establish chronology. A total-order count includes the current order; calling that count prior orders is unsupported. A supplied profile total is authoritative even when a detailed list is only a subset. 'No X documented/in merchant records' means no X in the supplied records, not that X never occurred. An absence statement scoped to supplied communications passes when no such message is present. An unqualified claim that no event occurred or no request was initiated does not acquire that scope merely because the output discusses records elsewhere. Check the actual sentence; do not add missing qualifications. A missing return request does not establish that no return was initiated. A null signed_by only establishes no signature recorded, not that none was obtained. Strategy decision rules are not retrieved merchant policy; numeric approval thresholds do not establish an unstated concession rule. A charge object or one shipment record does not establish that there were no other charges or shipments.
...
B. Event dates and ordering: first verify that EVERY stated event date belongs to that exact event/entity. An order's created_at supports 'the order was placed on that date', but NEVER 'the disputed charge was placed on that date'; shared purchase context does not supply a missing charge timestamp. Likewise a digital delivery timestamp is not a renewal timestamp. Reject such event substitution even when no before/after words occur. Then check ordering: asserting A occurred before/after B requires BOTH recorded event times or an explicit statement of that ordering. A deadline, identifier, order date, or delivery date cannot provide a missing dispute creation time. An attributed message reporting chronology only needs that message, not independent proof of the reported events.
C. Policy premises: an assertion that merchant policy or operational parameters permit a concession requires a stated concession rule applicable to this customer. An approval_amount_cents limit regulates approval, not reasons for concession. A VIP concession threshold does not cover a new customer; an always_concede_under_cents rule does not cover an amount above that limit. Do not invent discretionary or non-delivery concession rules from unrelated numbers. A literal supplied rule 'Concede when delivery cannot be confirmed' can support that policy statement; a bare recommendation makes no policy claim. Also check disclosure: populated refund_policy_disclosure and cancellation_policy_disclosure fields mean this customer was shown that policy before purchase. The source must establish disclosure, not merely policy contents or a general rule. Cancellation and refund/return policy types are distinct. A policy accurately quoted in narrative or uncategorized_text does not itself claim pre-purchase disclosure and passes this check.
...
For each category, first compare the exact relevant output words with the required source premises; if no claim of that category is present, pass it. Do not infer any missing premise. Return JSON with explanation first (cite exact unsupported field/claim and missing premise, or explain why relevant claims pass), then independent booleans event_absence_pass, event_order_pass, policy_disclosure_pass. Explicit goals and bare action recommendations do not assert event completion or chronology."""
    ...
    judge_valid = all(type(verdict.get(key)) is bool for verdict, keys in [
        (narrative_judge, ("reason_code_pass", "must_cite_pass")),
...
def main():
    ...
    def save_results(completed):
        json_file.write_text(json.dumps({
            ...
            "rubric": "grounded-v17", "completed": completed, "results": results,
        }, indent=2) + "\n", encoding="utf-8")
    save_results(False)
    action_matches = 0
    gate_matches = 0
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        ...
        f.write("**Rubric:** grounded-v17 (grounding across all strategy and evidence fields; reason, must-cite, and word count apply to narrative only). Gate measures hook interrupt request only.\n\n")
